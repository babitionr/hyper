...
const Page = () => {
  ...
  const getAllServiceCategory = async (param) => {
    const data = await MasterDataService.getAllServiceCategory({ ...param, type: 'MATERIAL' });
  ...
  const handSubmitForm = async () => {
    ...
    if (data) {
      if (value.uuid) {
        Message.success({ text: 'Chỉnh sửa nhóm vật tư thành công.', title: 'Thành Công', cancelButtonText: 'Đóng' });
      }
      if (!value.uuid) {
        Message.success({ text: 'Thêm nhóm vật tư thành công', title: 'Thành Công', cancelButtonText: 'Đóng' });
      }
    }
  ...
  const deleteSupply = async (uuid) => {
    const res = await MaterialMedicineService.delete(uuid);
    if (res) {
      Message.success({ text: 'Xóa vật tư thành công.', title: 'Thành Công', cancelButtonText: 'Đóng' });
      handleChange();
    }
  };
  const activeSupply = async (data) => {
    const res = await MaterialMedicineService.activeMaterialMedicine(data?.uuid);
    if (res) {
      if (data?.active) {
        Message.success({ text: 'Khóa vật tư thành công.', title: 'Thành Công', cancelButtonText: 'Đóng' });
      }
      if (!data?.active) {
        Message.success({ text: 'Mở khóa vật tư thành công.', title: 'Thành Công', cancelButtonText: 'Đóng' });
      }
      handleChange();
    }
  };
  ...
  const [handleChange, DataTable] = HookDataTable({
    Get: getData,
    columns: ColumnServices({ handleToggleAddNewService, deleteSupply, activeSupply }),
    loadFirst: false,
    ...
    newRowHeader: (
      <div className="flex gap-3 flex-col sm:flex-row pb-2 justify-between my-2">
        <button
          ...
          onClick={() => {
            !selectedServices
              ? Message.error({ text: 'Vui lòng chọn nhóm vật tư để thêm mới.' })
  ...
  return (
    <div className="bg-white rounded-lg py-3 overflow-scroll services__wrap">
      <div className="flex justify-between border-b border-blue-50 pb-4 ">
        <div className="ml-3 font-semibold text-lg text-gray-900 ">{'Thông tin vật tư'.toUpperCase()}</div>
      </div>
      {toggleAddNew === false ? (
        <div className="flex p-3 gap-4 z-50">
          <div className="w-1/3 border border-gray-200">
            <div className="border flex justify-between items-center h-14 bg-gray-200">
              <div className="text-sm 2xl:text-lg font-bold text-gray-600 p-2">Nhóm vật tư</div>
              <div className="pr-2">
                <button
                  className="pr-2 w-10 h-9 flex xl:w-24 xl:h-10 active:ring-2 ring-offset-1 ring-offset-red-300 ring-red-300  bg-red-500 text-white rounded-lg border border-red-500  items-center !text-base !font-medium px-3 xl:px-4 xl:py-2"
                  type="button"
        ...
        <Modal
          ...
          title={
            <div className="flex justify-between">
              <div className="text-base font-bold">Thêm nhóm vật tư</div>
              <button
                className=""
                onClick={() => {
                  handleCancel();
              ...
              <div className="px-4">
                <div>
                  <div className="w-full flex justify-between gap-4 pb-1">
                    <Form.Item className="w-full" label="Tên nhóm vật tư" name="name">
                      <Input
