...
import { ColumnReportAppointmentForDoctor } from '../columns/columnReportAppointmentForDoctor';
import { ReportService } from 'services/report';
import { useAuth } from 'global';
import { useSearchParams } from 'react-router-dom';
import * as XLSX from 'xlsx';

export const ReportAppointmentForDoctor = ({ appointmentScheduleReportOverview, filter, setFilter }) => {
  ...
  const [handleChange, DataTable] = HookDataTable({
    ...
    setIsLoading,
    showSearch: false,
    columns: ColumnReportAppointmentForDoctor({}),
  ...
  const handleExport = async (data) => {
    ...
    const Heading = [['Bác sĩ', 'Tổng số lịch hẹn', 'Đã thực hiện', 'Chưa thực hiện', 'Đã hủy']];
    const wb = XLSX.utils.book_new();
    XLSX.utils.sheet_add_aoa(wb, Heading);
    const ws = XLSX.utils.sheet_add_json(wb, value, { origin: 'A2', skipHeader: true, skipcolumn: 1 });
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'Thống kê theo bác sĩ.xlsx');
  };
