...
import { columnRevenueByBranch } from '../columns/ColumnRevenueByBranch';
...
function Page({ type }) {
  ...
  const calculateSummary = useCallback(() => {
    const total = listRecept.reduce((a, b) => a + b.receiptAmount, 0);
    return {
      total,
    };
  }, [listRecept]);
  ...
  return (
    <div>
      ...
      <div className="border border-gray-200 rounded-lg p-4 mb-4">
        ...
        <RevenueByBranch dataChart={listRecept} chartType="recept" />
      </div>
      <div>{DataTable()}</div>
    </div>
  );
