      ...
      if (data.message) {
        if (isComplete) {
          await Message.success({
            text: 'Xuất phiếu nhập kho thành công',
            title: 'Thành Công',
            cancelButtonText: 'Đóng',
          });
        } else if (!uuid) {
          await Message.success({
            text: 'Tạo mới phiếu nhập kho thành công',
            title: 'Thành Công',
            cancelButtonText: 'Đóng',
          });
        } else if (uuid) {
          await Message.success({
            text: 'Chỉnh sửa phiếu nhập kho thành công',
            title: 'Thành Công',
            cancelButtonText: 'Đóng',
          });
        }
      }
      navigateToImportStock();
      return data;
    } catch (error) {
      console?.error(error);
    }
        ...
        <div>
          <div className="ml-3">
            ...
            <span
              ...
              className="cursor-pointer"
            >
              <span className="hover:text-rose-500">Phiếu nhập kho</span> &gt;
            </span>{' '}
            <span className="cursor-default"> Thêm mới</span>
          </div>
          <div className="ml-3 pt-2 font-bold text-lg text-gray-900 "> Thêm mới phiếu nhập kho</div>
        </div>
