...
import { ColumnPassword } from './columns/index';
...
const Page = () => {
  ...
  const changePassword = async (values) => {
    try {
      ...
      const res = await AuthSerivce.changePass(values);
  ...
  const submit = async (values) => {
    try {
      ...
      // },500)
      // res &&
      //   Message.success({ text: `Lưu thông tin thành công!` }).then((result) => {
  ...
  return (
    <Fragment>
      <div className="min-h-screen">
        <p className="font-bold text-xl text-black mb-5">{'Thông tin cá nhân'.toUpperCase()}</p>
        <div className="flex flex-col bg-white w-full lg:flex-row lg:justify-center lg:bg-gray-100 user-detail-general-form">
          ...
          <div className="bg-white rounded-xl w-full profile">
        ...
        <div className="flex sm:flex-row flex-col items-center justify-end mt-7 ">
          ...
          {tabKey === '1' && (
            <button
              ...
              className="px-4 bg-teal-900 text-white text-sm p-2 rounded-[0.625rem] hover:bg-teal-600 mt-2 sm:w-auto w-[60%] h-[44px]"
              id="submitBtn"
            >
              Lưu thông tin
            </button>
          ...
          {tabKey === '2' && (
            <button
              ...
              className="px-4 bg-rose-500 text-white text-sm p-2 rounded-[0.625rem] mt-1 sm:w-auto w-[60%] h-[44px]"
              id="submitBtn"
            >
              Đổi mật khẩu
            </button>
