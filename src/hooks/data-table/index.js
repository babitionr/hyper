    ...
    (data, columns) => {
      ...
      return (
        <div className={classNames(className, 'intro-x')}>
          <div className="flex justify-between mb-2.5 flex-col sm:flex-row">
            {!!showSearch && (
              <div className="relative">
                <input
                  id={idE.current + '_input_search'}
                  className="sm:w-80 w-full inline-block h-10 rounded-xl text-gray-600 bg-white border border-solid border-gray-100 pr-9 pl-8 hook-table-search"
            ...
            {!!rightHeader && <div className="my-0 lg:my-0 text-left">{rightHeader}</div>}
          </div>
          {!!newRowHeader && <div>{newRowHeader}</div>}
              ...
              <Table
                ...
                columns={
                  ...
                  _columns.map((item, index) => {
                    if (item && cols.current[index]?.width) {
